...
- Types use `PascalCase` as per TypeScript convention.
...
e.g.: `id`, `px`, `ms` or `props`<br/>
- ✅: Do use plurals to identify lists (`Array`/`Map`/`Set`).<br/>
e.g.: `items` can be assumed to be an `Item[]`, not a `number`.<br/>
...
Functions _do things_, try prefixing with a verb: e.g. `user()` vs `createUser()`<br/>
Numbers often _count things_, e.g. `users` vs `userCount`.<br/>

# Props

## Naming

- 💡: [General naming conventions](#naming) apply.<br/>
...
e.g.: `setUser={setUser}` vs `onUserEdited={setUser}`<br/>
💡: The child component ideally doesn't care what the state manager does with the event data. See [dumb components](#TODO).

# Hooks

- 💡: [General naming conventions](#naming) apply.<br/>
- ✅: Do prefix all hooks with `use`. Enforced by [ESLint](https://www.npmjs.com/package/eslint-plugin-react-hooks).
